...
function validateevent() {
    var Fuser = document.getElementById('event').value;
    var regName = /^[A-Za-z_ ]+$/;
    if (Fuser == "" || Fuser.length < 2) {
        document.getElementById('errevent').style.display = "block";

        document.getElementById('errevent').innerHTML = divs[0];
        return false;
    } else if (!Fuser.match(regName)) {
        document.getElementById('errevent').style.display = "block";
        document.getElementById('errevent').innerHTML = divs[1];
        return false;
    } else {
